...
import { approveJobSubmission, deleteJobSubmission } from "@/lib/actions";
...
function ApproveSubmissionButton({ jobId }: AdminButtonProps) {
  const [formState, formAction] = useFormState(approveJobSubmission, undefined);
...
function DeleteJobButton({ jobId }: AdminButtonProps) {
  const [formState, formAction] = useFormState(deleteJobSubmission, undefined);
